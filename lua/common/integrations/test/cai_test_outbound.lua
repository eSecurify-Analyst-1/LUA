...
function Main:get_gdf_path_for_first_turn(tts_engine)
    if string.find(tts_engine, "no_tts") ~= nil then
        return LUA_DIR .. '/common/integrations/providers/google/gdf/text_to_voice.lua'
    else
        return LUA_DIR .. '/common/integrations/providers/google/gdf/text_to_text.lua'
    end
end
...
function Main:run()
    ...
    --logger:msg(LEVEL_INFO, "STARTED CALL AFTER RING READY")
    ...
    -- Init the logger
    local fs_env = true
    local logger = Logger:new { fs_env = fs_env, callee_phone_number = caller_phone_number, script_name="kavyaa_test_outbound_script.lua" , attempt_id=SESSION_ID}
    logger:msg(LEVEL_INFO, "LOGGER STARTED")
    ...
    logger:msg(LEVEL_INFO, "BEFORE FETCHING DATA")
    ...
    stmt:finalize()
    local length = string.len(destination_phone_number)
    destination_number_raw = string.sub(destination_phone_number,length-9)
    local user_data_query = 'SELECT * FROM users_data where user_number="' .. destination_number_raw .. '"'
    local stmt = db:prepare(user_data_query)
    local result = stmt:step()
    if result == sqlite3.ROW then
        parameters = stmt:get_value(1)
    end
    stmt:finalize()

    DYNAMIC_VALUES = json:decode(parameters)
    logger:msg(LEVEL_DEBUG, "PRINTING DYNAMIC VALUES")
    logger:msg(LEVEL_DEBUG, DYNAMIC_VALUES)
    logger:msg(LEVEL_INFO, "AFTER FETCHING DATA")
    STT = dofile(BASE_ENGINE_PATH .. stt_engine)
    TTS = dofile(BASE_ENGINE_PATH .. tts_engine)
    NLUEngine = dofile(BASE_ENGINE_PATH .. nlu_engine)
    logger:msg(LEVEL_INFO,TTS)
    stt = STT:new()
    nlu = NLUEngine:new({agent_id=nlu_uuid, session_id=SESSION_ID, language=LANGUAGE, sys_params=DYNAMIC_VALUES})
    tts = TTS:new()
    logger:msg(LEVEL_INFO, "AFTER INITIALIZING ENGINES")
    i=0
    allowed=50
    ...
    gdf_path_for_first_turn = Main:get_gdf_path_for_first_turn(tts_engine)
    logger:msg(LEVEL_DEBUG, "FIRST TURN PATH: " .. gdf_path_for_first_turn)
    NLUEngine1 = dofile(gdf_path_for_first_turn)
    ...
    logger:msg(LEVEL_INFO, "AFTER SETUP DATA IN GDF")
    logger:msg(LEVEL_INFO,STARTUP_TEXT)
    logger:msg(LEVEL_INFO,filepath)
    logger:msg(LEVEL_INFO, "BEFORE API CALL")
    local output_gdf = nlu1:process(STARTUP_TEXT, filepath)
    local final = tts:synthesize(output_gdf, filepath)

    logger:msg(LEVEL_INFO, "AFTER API CALL")
